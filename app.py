...
def send_request_to_arduino(hand_sign_id):
    arduino_ip = "192.168.27.86"
    if hand_sign_id == 0:
        url = f"http://{arduino_ip}/FORWARD/5"
    elif hand_sign_id == 1:
        url = f"http://{arduino_ip}/NOX/"
    elif hand_sign_id == 4:
        url = f"http://{arduino_ip}/DONE/"
    else:
        print(f"No action defined for hand_sign_id {hand_sign_id}")
        return

    try:
        response = requests.get(url, timeout=1)
        if response.status_code == 200:
            print(f"Successfully sent request for hand_sign_id {hand_sign_id}")
        else:
...
def send_request_to_arduino_async(hand_sign_id):
    thread = threading.Thread(target=send_request_to_arduino, args=(hand_sign_id,))
    thread.start()
    ...
    while True:
        ...
        if results.multi_hand_landmarks is not None:
            for hand_landmarks, handedness in zip(
                ...
                # Send request to Arduino only for specific hand_sign_id (0 or 1) and if it changes
                if hand_sign_id in [0, 1, 4] and hand_sign_id != last_hand_sign_id:
                    current_time = time.time()
                    if current_time - last_request_time > request_interval:
                        send_request_to_arduino_async(hand_sign_id)
        ...
        # send_request_to_arduino(hand_sign_id)
