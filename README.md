# MediaPipe Hand Gesture Recognition for WiFi Spider Control

This project leverages MediaPipe for hand gesture estimation and recognition (Python version), enabling control over a WiFi-enabled spider robot. It uses a simple MLP (Multilayer Perceptron) to recognize hand signs and finger gestures based on detected key points.
<br> 
...
This system integrates APIs written for Arduino and Raspberry Pi, enabling the control of a WiFi-enabled spider robot through recognized hand gestures. The seamless interaction between the gesture recognition system and the robotic hardware provides an intuitive and innovative method of control.

## Running the Demo
Use your webcam to run the demo:
```bash
python app.py
```
